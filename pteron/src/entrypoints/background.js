...
export default defineBackground(() => {
	...
	// Returns true if any saved link already points to the given URL.
	...
	// Returns the keyword of the first saved link that matches the given URL, or null.
	async function findLinkKeywordForUrl(url) {
		const target = normalizeForUrlComparison(url);
	...
	chrome.commands.onCommand.addListener(async (command) => {
		if (command === 'save-link') {
			...
			const existingKeyword = await findLinkKeywordForUrl(tabs[0].url);
			if (existingKeyword) {
			...
			const keyword = await findLinkKeywordForUrl(tabs[0].url);
			if (keyword) {
	...
	chrome.webNavigation.onCompleted.addListener(async (details) => {
		...
		if (!pending) {
			...
			const keyword = await findLinkKeywordForUrl(url);
			if (!keyword) return;
	...
	// Handle p/<keyword> navigation pattern so users can type "p/keyword" in the address bar
	// to jump directly to a saved link without the omnibox Tab trigger.
	// Covers two cases:
	//   1. Chrome navigates to http://p/<keyword> (local-hostname interpretation)
	//   2. Chrome searches Google for "p/<keyword>" (most common: no TLD → search)
