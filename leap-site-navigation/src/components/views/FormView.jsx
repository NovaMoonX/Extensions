...
export default function FormView({ editingKeyword, prefillData, pendingUrl, pendingTitle, onSaved, onCancel, onViewLeaflets, onViewAll }) {
  ...
  async function handleSubmit(e) {
    ...
    try {
      ...
      setMessage({ text: isEdit ? 'Lily Pad updated!' : 'Lily Pad saved!', type: 'success' });
      setTimeout(() => { onSaved(); }, 750);
    } catch {
      setMessage({ text: 'Error saving lily pad', type: 'error' });
    }
  }

  async function handleDelete() {
    if (!editingKeyword) return;
    if (!confirm(`Delete your lily pad "${editingKeyword}"?`)) return;
    await deleteLilyPad(editingKeyword);
    onCancel();
  }
  ...
  return (
    <div id="formView">
      <h2 id="formTitle">{isEdit ? 'Edit Lily Pad' : 'Add New Lily Pad'}</h2>
      ...
      <form onSubmit={handleSubmit}>
        ...
        {isEdit && (
          <button type="button" className="delete-link" onClick={handleDelete}>Delete This Lily Pad</button>
        )}
      </form>
      ...
      <button className="view-all" onClick={onViewAll}>View All Lily Pads</button>
      <ShortcutHint />
    </div>
  );
}
